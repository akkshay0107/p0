...
class LegalActionBuilder:
    ...
    def legal_actions(view: DecisionView, position: int) -> tuple[int, ...]:
        ...
        if slot.forced_move:
            ...
            mega_moves = tuple(action + (MOVE_END - MOVE_START) for action in moves) if slot.can_mega else ()
        return (*switches, *moves, *mega_moves) or (PASS_ACTION,)

    @classmethod
    def mask(cls, view: DecisionView) -> npt.NDArray[np.bool_]:
        mask = np.zeros((2, ACT_SIZE), dtype=np.bool_)
        for position in (0, 1):
            mask[position, cls.legal_actions(view, position)] = True
        return mask

    @staticmethod
    def validate_joint(view: DecisionView, first: int, second: int) -> bool:
        if first not in LegalActionBuilder.legal_actions(view, 0):
            return False
        return bool(LegalActionBuilder.second_mask(view, first)[second])

    @classmethod
    def second_mask(cls, view: DecisionView, first: int) -> npt.NDArray[np.bool_]:
        mask = np.zeros(ACT_SIZE, dtype=np.bool_)
        mask[list(cls.legal_actions(view, 1))] = True
        if view.team_preview:
            try:
                first_pair = ActionCodec.decode_team_pair(first, view.team_size)
            except ValueError:
                mask.fill(False)
                return mask
            for action in np.flatnonzero(mask[:36]):
                try:
                    second_pair = ActionCodec.decode_team_pair(int(action), view.team_size)
                except ValueError:
                    mask[action] = False
                    continue
            ...
            semantic = ActionCodec.decode(first)
