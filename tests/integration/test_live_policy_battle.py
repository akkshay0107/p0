...
class TrackedPolicyPlayer(RLPlayer):
    ...
    def _get_action(self, battle):
        ...
        # Track the latest resident recurrent memory token created for this battle step
        ...
        # Stash chosen action tuple to verify round-trip conversion after order generation
    ...
    def _record_order_round_trip(self, battle: DoubleBattle, order: BattleOrder) -> None:
        """Validate that the Showdown wire order string encodes back to the selected action tuple."""
...
async def test_checkpoint_free_policy_completes_live_battle(
    ...
    3. Battle history and memory states are cleaned up completely upon battle termination.
    """
    torch.manual_seed(7)
    ...
    # Verify battle history dictionary was purged upon match completion to prevent memory leaks
    ...
    if isinstance(second, TrackedPolicyPlayer):
        ...
        # Ensure distinct players maintained independent recurrent memory streams
...
async def test_model_policy_round_trips_live_preview_and_action_orders(
    ...
    """Verify that every order generated by RLPlayer reconstructs the exact discrete action pair."""
    ...
    # Ensure both team-preview and in-battle actions were exercised in the round-trip check
    ...
    # Selected action tuple (slot 0, slot 1) must identically match encoded wire order action tuple
