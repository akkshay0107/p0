...
class _ObservationCapturePlayer(RandomPlayer):
    ...
    def _capture_observation(self, battle: AbstractBattle, callback: str) -> None:
        try:
            ...
            # Capture builder failures without interrupting the live battle under observation.
...
class TestObservation:
    ...
    @pytest.mark.asyncio
    async def test_observation_builder_live(
        self, showdown_server, battle_format, sample_team
    ) -> None:
        """
        ...
        try:
            try:
                ...
                # Convert asynchronous player failures into an assertion with captured context.
                pytest.fail(
                    f"Battle failed with exception: {exc}. Internal errors: {captured_errors}"
                )
        finally:
