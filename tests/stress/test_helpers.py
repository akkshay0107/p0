...
class TestHelpers:
    @pytest.mark.stress
    def test_stress_batch_sizes_deduplicate_without_reordering(self) -> None:
        """Verify that batch size string parser removes duplicates while preserving original order."""
        assert stress_batch_sizes("8, 1, 8, 32") == (8, 1, 32)

    @pytest.mark.stress
    @pytest.mark.parametrize("value", ["", "0,2", "1,-1"])
    def test_stress_batch_sizes_reject_invalid_values(self, value: str) -> None:
        """Verify that empty, zero, or negative batch sizes raise ValueError."""
        with pytest.raises(ValueError, match="P0_STRESS_BATCHES"):
            stress_batch_sizes(value)

    @pytest.mark.stress
    def test_stress_int_enforces_the_configured_minimum(self) -> None:
        """Verify that stress_int raises ValueError when parsed env integer violates the minimum threshold."""
        with pytest.raises(ValueError, match="at least 3"):
            stress_int("P0_STRESS_CONTROL", 1, minimum=3, raw_value="2")

    @pytest.mark.stress
    def test_random_team_records_stay_inside_the_active_dex(self) -> None:
        """
        ...
        records = tuple(
            stress_random_team_record(rng, label=f"helper-{index}") for index in range(32)
        )
    ...
    @pytest.mark.stress
    def test_random_replay_payload_builder_rejects_empty_workloads(self) -> None:
