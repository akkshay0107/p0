"""Test observation serialization, legality provenance, and state isolation."""
...
class TestObservationBuilder:
    def test_observation_builder_serializes_pokemon_features(self) -> None:
        ...
        battle = make_battle_view(
            active_pokemon=[mon, None], team=[mon], can_mega_evolve=[True, False]
        )
    ...
    def test_concurrent_universal_effect_stress_state(self) -> None:
...
class TestObservationBuilderLegalityAndState:
    def test_unknown_legality_is_gated_rather_than_written_as_illegal(self) -> None:
    ...
    def test_reconstructed_observations_clear_reused_buffer_state(self) -> None:
        ...
        for snapshot in perspective.snapshots:
            ...
            assert output.numerical[12, NUM_IDX_TEAM_PREVIEW].item() == float(
                snapshot.view.teampreview
            )
