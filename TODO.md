...
- [ ] Update the vocab, observation builder and model to support the entirety of VGC.
- [ ] Create a pipeline for cloning behaviour from public showdown replays as a replacement for the heuristic bootstrapping.
- [ ] Maybe consider a policy guided MCTS approach (have to build an engine from scratch) since it can utilise all of the time given per turn. (??)
- [ ] Run a much larger training job to try to reach parity in performance with the top players.

If you are interested in this project, want to contribute, or know of any resources where I could potentially train a larger model for cheap, please reach out. The long term goal would be to have a model that is able to adapt to different regulation sets as they come out with minimal retraining. Ideally, I would like for it to be at the level where people should be able to use this for practice to try new lines / teams at a pro level.
